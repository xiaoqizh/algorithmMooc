 ...
 *           并查集所消耗的时间 主要是从下到上的查的过程中消耗时间
 *           connect()操作消耗时间也是在find()方法消耗的
 */

public class UnionFind2PathCompress {
    /**
     *      在树的深度上进行优化 就是压缩路径
     *      其实使用rank()的形式进行优化已经很好了
     */
    private int[] parent;
    private int[] size;
    private int count;

    UnionFind2PathCompress(int n) {
        count = n;
        parent = new int[n];
        size = new int[n];
        for (int i = 0; i < n; i++) {
